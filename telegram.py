...
# Указываем токен для телеграма для управления

bot = telebot.TeleBot('1083811573:AAFZjvKkzNq0p3ImTdVEKfWwqB1DfTCihAw')
...
def get_text_messages(message):
    ...
    if message.text == "Привет":
        ...
        bot.send_message(message.from_user.id, "Напиши Привет и все встанет на свои места")
    elif message.text == 'Hi':
        bot.send_message(message.chat.id, 'Привет, мой создатель')
    elif message.text == 'Bye':
        bot.send_message(message.chat.id, 'Прощай, создатель')
    else:
        bot.send_message(message.chat.id, 'Извини не понимаю- давай что-то делать! ')
...
def start_message(message):
    bot.send_message(message.chat.id, 'Привет, ты написал мне ЧТО БУДЕМ ДЕЛАТЬ /start')
